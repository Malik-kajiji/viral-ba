...
const reActive = async (req,res) => {
    const { _id } = req.body;
    try{
        const virus = await virusModel.updateIsLocked(_id,false)
...
const getAllVirusesRequest = async (req,res) => {
    try{
        const viruses = await requestModel.getVirusesRequests()
...
const deleteVirusRequest = async (req,res) => {
    const { _id } = req.body

    try{
        const virus = await requestModel.findOneAndDelete({_id})
...
module.exports = {getAllViruses,addVirus,editVirus,removeVirus,changePassword,addClientToVirus,removeClientFromVirus,getVirusClients,reActive,getAllVirusesRequest,deleteVirusRequest}